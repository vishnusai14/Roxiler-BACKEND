...
//Login to the admin DB
const url = `mongodb://${rootUsername}:${rootPassword}@localhost:27017/admin`;

const conn = new Mongo(url);
const db = conn.getDB(database);

// Create a new user with roles for the "stat (newly created)" database
db.createUser({
  user: dbUser,
  pwd: dbPass,
